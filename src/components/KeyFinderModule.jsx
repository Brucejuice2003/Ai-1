        ...
        const fetchItunesCover = async () => {
            try {
                ...
                if (data.resultCount > 0) {
                    ...
                    let artwork = data.results[0].artworkUrl100?.replace('100x100', '600x600') || data.results[0].artworkUrl100;
                    setSrc(artwork);
                }
            } catch (e) {
                // ignore
...
export default function KeyFinderModule() {
    ...
    useEffect(() => {
        // Load the 100 song reference list by default
        const demoData = getRandomSuggestions().map(s => ({
            ...s,
            source: 'mock',
            isDemo: true,
            // We let SmartCover handle the images
        }));
    ...
    const handleSearch = async (e) => {
        ...
        if (query.length === 0) {
            ...
            return;
            // Try to get real title/artist
            let title = "YouTube Video";
            let artist = "Click Play to Analyze Key";

            const metadata = await YouTubeService.getVideoMetadata(query);
            if (metadata) {
            ...
            setSearchResults([{
                ...
                key: "Live Analysis Required",
                bpm: "-",
                isReal: true,
                source: 'youtube', // This triggers playback logic
                isDemo: false
            }]);
            setIsSearching(false);
            return;
        ...
        // 2. Search Local Reference (Best source because it has curated Keys)
        const localResults = await searchSongs(query);
        const formattedLocal = localResults.map(s => ({ ...s, source: 'mock', isDemo: true }));

        // 3. Search iTunes Global Database (For "Any Song" covers)
        let globalResults = [];
        if (query.length > 2) {
            try {
                ...
                <div className="relative max-w-md w-full mx-4" onClick={(e) => e.stopPropagation()}>
                    ...
                    <div className="glass-panel p-6 space-y-4 animate-scale-in">
                        ...
                        <a
                            ...
                            className="block w-full bg-gradient-to-r from-red-600 to-red-500 hover:from-red-500 hover:to-red-400 text-white font-bold py-3 px-6 rounded-full transition-all transform hover:scale-105 shadow-lg text-center"
                        >
                            🔍 Search on YouTube
                        </a>
                    </div>
